...
class _AfiliadosCarouselState extends State<AfiliadosCarousel> {
  ...
  ScrollController controllerCarrousel = ScrollController();
  ...
  void initState() {
    ...
    loadData();
    super.initState();
    controllerCarrousel..addListener(() => {scrollListen()});
  }

  @override
  void dispose() {
    super.dispose();
    controllerCarrousel.removeListener(() => {scrollListen()});
  }

  scrollListen() async {
    if (controllerCarrousel.position.pixels ==
        controllerCarrousel.position.maxScrollExtent) {
      print("data mas");
    }
  }
  ...
  void loadMore() async {
    ...
    print("load more");
    afiliados.addAll(
        await service.loadByStart(afiliados[afiliados.length - 1].latitud));
    setState(() {});
  ...
  void loadData() async {
    ...
    if (widget.categoria != null) {
      afiliados = await service.loadByCategoria(widget.categoria.nombre);
      print(afiliados);
    } else {
      ...
      child: LazyLoadScrollView(
        ...
        child: Scrollbar(
          child: ListView.builder(
              controller: controllerCarrousel,
              ...
              itemBuilder: (context, index) {
                ...
                return _containerAfiliado(context, afiliados[index]);
              }),
        ),
      ),
    ));
  ...
  Widget _containerAfiliado(BuildContext context, Afiliado a) {
    String url = a.img.contains("http") ? a.img : "https:" + a.img;
        ...
        child: Column(
          children: [
            Container(
                ...
                width: _size.width * 0.8,
                child: a.img == null
                    ? Image.asset("assets/imgnotfound.png", fit: BoxFit.cover)
